...
│   ├── weather_hourly.py  # Stage W: NOAA LCD v2 hourly weather -> GCS -> BigQuery
...
├── sql/station_counts_monthly.sql # monthly stations + trips per region (mirrors analytics.py)
...
Five stages, each a CLI module run via the `Makefile`. Cloud auth is automatic
...
| W — hourly weather | `make weather-hourly` | Mirror NOAA LCD v2 hourly CSVs for Central Park into `raw/lcd/`, extract typed Parquet, deploy `weather.weather_hourly_nyc` + its `m_` snapshot. See **Hourly NYC weather** below. |
...
**Network size (`station_counts_monthly`, `make stations`).** To read ridership *per station*
— separating real usage growth from the network ~7×'ing its coverage — we count distinct
physical stations per month. The reconciliation key matters: station **ids** churn across the
2021 schema change (legacy integers `497` vs current strings `5905.14`), so a distinct-id count
jumps 2,779→1,750→2,350 over 2021–2023 with no real change; and current-era **coordinates** are
per-trip *bike GPS*, not a fixed anchor, so rounded lat/long explodes to >12k phantom stations a
...
## Hourly NYC weather (NOAA LCD v2)

Stage W (`src/citibike_pipeline/weather_hourly.py`, `make weather-hourly`) ingests the
*hourly* record of the same Central Park station the daily mart summarizes: NOAA **Local
Climatological Data v2** for GHCN station `USW00094728` — one CSV per station-year from
`ncei.noaa.gov/oa/local-climatological-data/v2/access/`, ~24 routine FM-15 METARs/day
plus FM-16 specials (the files' SOD/SOM daily/monthly summary rows are dropped at
extract; dailies are `weather_daily_nyc`'s domain). The flow mirrors the trips pipeline:
immutable raw CSVs at `gs://citibike-archive/raw/lcd/`, typed SI Parquet at
`weather/lcd/parquet/`, then in `nyu-datasets.weather`: external **`lcd_hourly_nyc`** →
view **`weather_hourly_nyc`** (imperial conversions, local-time keys, condition flags) →
snapshot **`m_weather_hourly_nyc`** (~151k obs, 2013 → present). The view SQL is
mirrored in `sql/weather_hourly_nyc.sql`.

Pinned source facts (`make selftest` covers the value-marker rules):

- **Time basis.** LCD v2 timestamps are local *standard* time year-round (UTC−5, never
  DST) — verified by matching identical observations against the UTC-stamped ISD record
  in both January and July. Join trips on **`obs_time_local`** (LST → UTC →
  America/New_York), never on `obs_time_lst`, which is an hour behind all summer.
  Sanity pin: the hottest average local hour is 13:00–14:00 (solar noon ≈ 13:00 EDT).
- **Units.** LCD v2 is SI (°C, mm, m/s, km, hPa). Parquet keeps SI faithfully; the view
  adds the daily mart's imperial conventions (°F, inches, mph, miles).
- **Value markers.** `T` trace precipitation → 0.0 (as GHCN daily treats trace);
  trailing `s` = NCEI "suspect" QC flag → NULL (hourly analog of the daily mart's
  `qflag IS NULL`); trailing `V` = "variable" → value stands; `VRB` wind direction → NULL.
- **Snow.** There is *no hourly accumulation source anywhere* — snowfall/depth are
  once-daily manual measurements (use the daily mart). Hourly snow appears only as the
  `is_snowing` present-weather flag.
- **Flags.** `is_foggy`/`is_thunder`/`is_hazy` mirror the daily WT01/WT03/WT08 semantics
  (`FG` not `BR`); `is_raining`/`is_snowing` mean *falling now* — the daily
  `is_rainy`/`is_snowy` mean "accumulated today", a different statement.
- **Coverage bonus.** Dew point / RH / wet-bulb are ≥99% populated from **2013** here,
  vs. 2016+ in the daily mart — the humidity record now spans the whole Citibike era.

Cross-checked against `m_weather_daily_nyc` over 4.7k shared days: daily mean of the
hourly temps correlates 0.998 with `tavg_f` (mean |Δ| 0.85°F — expected, `tavg` is
(tmax+tmin)/2, not the 24h mean) and day-summed hourly precip correlates 0.993 with
`prcp_inches`. A standalone copy of this ingester is kept in the reference repo at
`ipeirotis-org/datasets` under `Citibike/`.
