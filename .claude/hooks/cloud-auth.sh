...
# --- Trust the sandbox's TLS-inspecting proxy CA so gcloud/bq can reach Google APIs ---
if [ -f /etc/ssl/certs/ca-certificates.crt ]; then
  gcloud config set core/custom_ca_certs_file /etc/ssl/certs/ca-certificates.crt 2>/dev/null || true
fi
