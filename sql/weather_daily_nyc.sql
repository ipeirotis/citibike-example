...
(
  ...
  prcp_inches OPTIONS(description="Total daily precipitation (rain plus melted snow) in inches (GHCN-D PRCP); NULL when not reported (a dry day is 0)."),
  snow_inches OPTIONS(description="Daily new snowfall in inches (GHCN-D SNOW); NULL when not reported. New snowfall, not depth on the ground."),
  is_rainy OPTIONS(description="1 if measurable precipitation fell, 0 if dry, NULL when precipitation was not reported."),
  is_snowy OPTIONS(description="1 if measurable snow fell, 0 if none, NULL when snowfall was not reported."),
  is_hot_day OPTIONS(description="1 if the daily high exceeded 90 degrees F, 0 if not, NULL when no maximum temperature was reported."),
  is_freezing OPTIONS(description="1 if the daily low was below freezing (32 degrees F), 0 if not, NULL when no minimum temperature was reported."),
