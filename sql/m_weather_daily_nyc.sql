...
ALTER TABLE `nyu-datasets.weather.m_weather_daily_nyc`
  ...
  ALTER COLUMN prcp_inches SET OPTIONS(description="Total daily precipitation (rain plus melted snow) in inches (GHCN-D PRCP); NULL when not reported (a dry day is 0)."),
  ALTER COLUMN snow_inches SET OPTIONS(description="Daily new snowfall in inches (GHCN-D SNOW); NULL when not reported. New snowfall, not depth on the ground."),
  ALTER COLUMN is_rainy SET OPTIONS(description="1 if measurable precipitation fell, 0 if dry, NULL when precipitation was not reported."),
  ALTER COLUMN is_snowy SET OPTIONS(description="1 if measurable snow fell, 0 if none, NULL when snowfall was not reported."),
  ALTER COLUMN is_hot_day SET OPTIONS(description="1 if the daily high exceeded 90 degrees F, 0 if not, NULL when no maximum temperature was reported."),
  ALTER COLUMN is_freezing SET OPTIONS(description="1 if the daily low was below freezing (32 degrees F), 0 if not, NULL when no minimum temperature was reported."),
